...
const StatHeader = () => {
  ...
  const { knowledgeStat, verticesStat, edgesStat, propertiesStat } = useMemo(() => {
    ...
    const knowledgeStatValue = verticesStat.value + edgesStat.value + propertiesStat.value;
    const knowledgeStatChanged = verticesStat.changed + edgesStat.changed + propertiesStat.changed;
    const knowledgeStatPercentage = knowledgeStatChanged / (knowledgeStatValue - knowledgeStatChanged) * 100;
    const knowledgeStat = {
  ...
  const isKnowledgeStatLoading = verticesStatQuery.isLoading || edgesStatQuery.isLoading || propertiesStatQuery.isLoading;
...
const StatCell = ({ label, value = 0, percentage = 0, isLoading = false }: StatCellProps) => {
  return <Stat p={4} borderRadius={"xl"} borderWidth={1}>
    <StatLabel>
